...
/**
 * @brief: friend an operator of << used to format Carton length_ x width_ and height_
 * @param out: a reference to the ostream that we are using to output
 * @param c: a reference of the Carton object
 * @return out: The formatted data
 */
std::ostream &operator<<(std::ostream& out, const Carton &c)
{
    out << std::fixed << std::setprecision(1) << "Carton " << c.length_ << " x " << c.width_ << " x " << c.height_;
    return out;
}
